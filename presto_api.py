...
class PrestoAPI:
    ...
    def __init__(self):
        ...
        # Меню для загрузки
        self.menus = {
            169: "🍳 ЗАВТРАКИ (Пн-пт до 13:00)",
            167: "🥞 ЗАВТРАКИ (Сб-Вс до 16:00)",
            166: "📋 МЕНЮ КУХНЯ",
            141: "🧀 СЫРНАЯ КАРТА",
            159: "🍷 БАРНАЯ КАРТА",
            162: "🥤 НАПИТКИ"
    ...
    async def get_all_menus(self) -> Dict[int, Dict]:
        ...
        # 159: Бар
        target_menu_ids = {169, 167, 166, 141, 162, 159}

        # Создаем словарь priceLists по ID для быстрого поиска
        price_lists_dict = {int(pl['id']): pl for pl in price_lists}

        # Загружаем меню для каждого ID
        for menu_id in target_menu_ids:
            if menu_id not in price_lists_dict:
                logger.info(f"⚠️ Меню {menu_id} не найдено в текущих прайс-листах (возможно скрыто по времени), пробуем загрузить принудительно...")
                # Добавляем фейковую запись, чтобы get_menu_by_id использовал priceListId
                price_lists_dict[menu_id] = {'id': menu_id, 'name': self.menus.get(menu_id, f"Меню {menu_id}")}
