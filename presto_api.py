...
class PrestoAPI:
    ...
    def calculate_delivery_cost_simple(self, district: Dict, cart_total: float, original_cart_total: float = None) -> Tuple[float, str]:
        ...
        try:
            ...
            # Форматируем текст
            if delivery_cost == 0:
                logger.info(f"🎉 Доставка бесплатная (заказ от {free_delivery_threshold}₽)")
                return 0.0, f"🎉 Бесплатно"
            else:
                return float(delivery_cost), f"{delivery_cost:.0f}₽ (бесплатно от {free_delivery_threshold}₽)"
