...
🚚 <b>Доставка</b> — заказ блюд на дом
...
Вы можете задать любой вопрос оператору, нажав кнопку <b>"Позвать человека"</b>, или ответьте на уточняющие вопросы сначала, и наш оператор уже будет знать, что вы хотите.
...
async def show_restaurant_menu(user_id: int, bot):
    """Показать меню ресторана (как команда /menu)"""
    try:
        # Текст как в handlers_delivery.menu_food_handler
        text = """🍽️ <b>Меню ресторана</b>
...
async def handle_text_messages(message: types.Message, state: FSMContext):
    ...
    # 🛑 СПЕЦИАЛЬНЫЙ ОБРАБОТЧИК: "Настройки"
    if text == "настройки" or text == "settings":
        # Проверяем, админ ли это
        admins = database.get_all_admins()
        if user.id in admins:
             # Импортируем клавиатуру настроек из admin_handlers
             # Но проще просто отправить сообщение с предложением открыть админку
             kb = types.InlineKeyboardMarkup(inline_keyboard=[
                 [types.InlineKeyboardButton(text="⚙️ Админ-панель", callback_data="admin_menu")]
             ])
             await safe_send_message(message.bot, user.id, "⚙️ <b>Настройки бота</b>\n\nВы администратор. Нажмите кнопку ниже, чтобы открыть панель управления.", parse_mode="HTML", reply_markup=kb)
             return
        else:
             # Для обычных пользователей - шутка или инфо
             await safe_send_message(message.bot, user.id, "🤖 Мои настройки просты: кормить вкусно, отвечать быстро! 😄\n\nЕсли у вас есть пожелания по работе бота — напишите нам!", parse_mode="HTML")
             return
    ...
    # ОТКЛЮЧЕНО: Теперь приветствия обрабатываются через AI для более живого общения
    ...
    try:
        ...
        if result.get('show_category_brief'):
            ...
            await handle_show_category_brief(category_name, user_id, message.bot, intro_message=clean_text)
        ...
        if result.get('show_category'):
            ...
            from category_handler import handle_show_category
            await handle_show_category(category_name, user_id, message.bot, intro_message=clean_text)
        ...
        # Проверяем на показ главного меню бота
