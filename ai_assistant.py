...
async def get_ai_response(message: str, user_id: int) -> dict:
    ...
    try:
        ...
        system_prompt = (
            ...
            f"'Мы постоянно обновляем наше меню, чтобы радовать вас вкусными блюдами! 😋 Рекомендую посмотреть наши хиты в разделах Пицца или Горячее!'\n"
            f"И добавь маркер: PARSE_CATEGORY:пицца\n"
            f"ЗАПРЕЩЕНО спрашивать 'Хотите посмотреть, что появилось?', так как у тебя НЕТ списка новинок! Сразу предлагай конкретные категории!\n\n"
