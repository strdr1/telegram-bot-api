...
async def get_ai_response(message: str, user_id: int) -> Dict:
    ...
    try:
        ...
        system_prompt = (
            ...
            f"5. Если пользователь уже увидел список блюд (пиццы, морепродуктов, салатов, завтраков и т.п.) и задаёт 'Что посоветуешь?', 'Какую посоветуешь?' или похожий вопрос — выбери 1–3 подходящих блюда из ЭТОГО списка и кратко порекомендуй их (без повторения всего списка и без новых маркеров PARSE_CATEGORY/SEARCH).\n"
