...
async def get_ai_response(message: str, user_id: int) -> Dict:
    ...
    try:
        ...
        system_prompt = (
            ...
            f"КРИТИЧЕСКИ ВАЖНО: Если спрашивают про ЧАСТНЫЕ МЕРОПРИЯТИЯ ('день рождения', 'отметить день рождения', 'свадьба', 'корпоратив', 'юбилей', 'празднование', 'банкет', 'организовать мероприятие', 'провести праздник', 'можно отметить') - НЕ СПРАШИВАЙ КОЛИЧЕСТВО ГОСТЕЙ!\n"
            f"Сразу показывай опции частных мероприятий!\n"
            ...
            f"ОБЯЗАТЕЛЬНО используй ТОЧНЫЙ маркер PARSE_CATEGORY:НАЗВАНИЕ_КАТЕГОРИИ для всех вопросов о категориях:\n"
            f"• 'А у вас есть пиццы?' -> ТОЛЬКО: PARSE_CATEGORY:пицца\n"
            f"• 'А пицца?' -> ТОЛЬКО: PARSE_CATEGORY:пицца\n"
            f"• 'У вас есть супы?' -> ТОЛЬКО: PARSE_CATEGORY:супы\n"
            f"• 'А суп?' -> ТОЛЬКО: PARSE_CATEGORY:супы\n"
            f"• 'Салаты какие есть?' -> ТОЛЬКО: PARSE_CATEGORY:салаты\n"
            f"• 'А салаты?' -> ТОЛЬКО: PARSE_CATEGORY:салаты\n"
            f"• 'Есть ли коктейли?' -> ТОЛЬКО: PARSE_CATEGORY:коктейли\n"
            f"• 'А коктейли?' -> ТОЛЬКО: PARSE_CATEGORY:коктейли\n"
            f"• 'Что из горячего?' -> ТОЛЬКО: PARSE_CATEGORY:горячее\n"
            f"• 'А горячее?' -> ТОЛЬКО: PARSE_CATEGORY:горячее\n\n"
