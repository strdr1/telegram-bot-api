...
async def get_ai_response(message: str, user_id: int) -> Dict:
    ...
    try:
        ...
        system_prompt = (
            ...
            f"На такие вопросы ОБЯЗАТЕЛЬНО используй ТОЧНЫЙ маркер PARSE_CATEGORY:НАЗВАНИЕ_КАТЕГОРИИ\n"
            f"Примеры:\n"
            f"- 'У вас есть пиво?' -> PARSE_CATEGORY:пиво\n"
            f"- 'Есть ли водка?' -> PARSE_CATEGORY:водка\n"
            f"- 'У вас есть салаты?' -> PARSE_CATEGORY:салаты\n"
            f"Используй названия категорий на русском языке, как они звучат в вопросе или в меню.\n\n"
