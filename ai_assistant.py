...
async def get_ai_response(message: str, user_id: int) -> Dict:
    ...
    try:
        ...
        system_prompt = (
            ...
            f"Примеры:\n"
            f"- 'Сколько калорий в пицце?' -> PARSE_CATEGORY:пицца (и спроси 'В какой именно пицце?')\n"
            f"- 'Какая калорийность у супов?' -> PARSE_CATEGORY:суп (и спроси 'В каком именно супе?')\n"
            f"- 'Сколько калорий в десертах?' -> PARSE_CATEGORY:десерт (и спроси 'В каком именно десерте?')\n"
