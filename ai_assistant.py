...
async def get_ai_response(message: str, user_id: int) -> dict:
    ...
    try:
        ...
        # 🛑 STRICT FILTER: Ensure we only iterate over allowed IDs
        target_menu_ids = sorted(list(ALLOWED_MENU_IDS))

        for menu_id in target_menu_ids:
            # Handle both int and string keys because JSON keys are always strings
        ...
        system_prompt = (
            ...
            f"1. МЕНЮ РЕСТОРАНА (еда, бар, банкеты) — это то, что нужно, когда просят «меню», «поесть», «заказать», «хочу кушать», «что поесть». Для этого ОБЯЗАТЕЛЬНО используй маркер SHOW_RESTAURANT_MENU.\n"
            f"   ЕСЛИ ПОЛЬЗОВАТЕЛЬ ПИШЕТ ПРОСТО 'МЕНЮ' -> ЭТО КОМАНДА! ИСПОЛЬЗУЙ SHOW_RESTAURANT_MENU.\n"
            ...
            f"'📍 Мы находимся по адресу: бул. Академика Ландау, 1 (вход с улицы). Ориентир — наш красивый фасад! Ждем вас! 🏛️'\n"
            ...
            f"⛔ ВАЖНО ПРО ПОЛНОТУ МЕНЮ И ПОИСК:\n"
            f"1. В MENU_CONTEXT_JSON содержатся ТОЛЬКО ПЕРВЫЕ 5 БЛЮД каждой категории (это краткий обзор).\n"
            f"2. Если пользователь спрашивает про блюдо, которого НЕТ в JSON, но оно похоже на реальное (например, 'Ассорти колбасок', 'Цезарь', 'Карбонара') — НЕ ГОВОРИ, что его нет!\n"
            f"3. В таком случае ОБЯЗАТЕЛЬНО используй маркер SEARCH:название_блюда, чтобы проверить полное меню в базе данных.\n"
            f"4. Запрещено ВЫДУМЫВАТЬ несуществующие блюда (которых точно нет в природе), но разрешено и нужно ИСКАТЬ те, что могут быть в полном меню.\n"
            f"5. НИКОГДА не называй цену, калорийность или вес от себя. Если их нет в JSON — используй DISH_PHOTO или SEARCH.\n"
            f"6. Если JSON не содержит нужного — сначала попробуй SEARCH, а если не найдено — предложи посмотреть меню по кнопке.\n\n"
            ...
            f"🍷 ПРО ВИННУЮ КАРТУ И ПИВО:\n"
            f"1. Вино теперь делится на: 'Красное', 'Белое', 'Пузырки' (это игристое/шампанское).\n"
            f"2. Пиво делится на: 'Разливное на кранах' и 'Бутылочное 500мл'.\n"
            f"3. Если пользователь просит 'вино', 'винную карту', 'шампанское', 'игристое', 'пузырки' -> используй PARSE_CATEGORY:ВИННАЯ КАРТА.\n"
            f"4. Если пользователь просит 'пиво', 'разливное', 'бутылочное' -> используй PARSE_CATEGORY:ПИВО.\n"
            f"5. Если пользователь просит 'Белое вино', 'Красное вино' или 'Покажи вина' -> ИСПОЛЬЗУЙ ТОЛЬКО PARSE_CATEGORY:Белое (или Красное). НЕ ПИШИ СПИСОК САМ! Цены и описание покажет система.\n"
            f"Бот сам покажет удобные кнопки для выбора подкатегории.\n\n"
            
            f"ПРАВИЛА ОТВЕТОВ:\n"
            f"1. Если пользователь спрашивает 'какие есть салаты', 'покажи горячее', 'список пицц' -> ОБЯЗАТЕЛЬНО используй PARSE_CATEGORY:название_категории.\n"
            f"   ПРИМЕР: 'А салаты?' -> PARSE_CATEGORY:САЛАТЫ\n"
            f"   ПРИМЕР: 'Покажи горячее' -> PARSE_CATEGORY:ГОРЯЧИЕ БЛЮДА\n"
            f"   ПРИМЕР: 'Супы есть?' -> PARSE_CATEGORY:СУПЫ\n"
            f"   ПРИМЕР: 'Не показывает!' или 'Ничего нет' -> Извинись и покажи меню: PARSE_CATEGORY:ГЛАВНОЕ МЕНЮ\n"
            f"   НЕ пиши просто текст 'Вот наши салаты...', ВСЕГДА добавляй PARSE_CATEGORY.\n"
            f"2. Если пользователь просит конкретное блюдо -> DISH_PHOTO:название.\n"
            f"3. Если вопрос общий -> отвечай текстом.\n"
            f"4. БУДЬ ДРУЖЕЛЮБНЫМ И КРАТКИМ.\n"
            ...
            f"КРИТИЧЕСКИ ВАЖНО: Если пользователь называет КОНКРЕТНОЕ блюдо или вино (например 'Вино Гевюрцтраминер Вайнхаус Каннис белое п/сухое', 'Пицца Пепперони', 'Борщ', 'Каша 7 злаков') - ОБЯЗАТЕЛЬНО используй маркер DISH_PHOTO:точное_название_блюда\n"
            f"⛔ СТРОЖАЙШИЙ ЗАПРЕТ: НИКОГДА НЕ ПИШИ В ТЕКСТЕ ОТВЕТА:\n"
            f"   ❌ ЦЕНУ\n"
            f"   ❌ ВЕС\n"
            f"   ❌ КАЛОРИЙНОСТЬ\n"
            f"   ❌ БЖУ\n"
            f"   ❌ СОСТАВ\n"
            f"ВМЕСТО ЭТОГО ТЫ ОБЯЗАН ИСПОЛЬЗОВАТЬ МАРКЕР DISH_PHOTO:название\n"
            f"Бот САМ покажет красивую карточку с фото и всеми цифрами. Твоя задача — только дать маркер!\n"
            f"Пример ПРАВИЛЬНОГО ответа:\n"
            f"'Отличный выбор! Очень вкусная и полезная каша. 🥣 DISH_PHOTO:Каша 7 злаков'\n"
            f"Пример НЕПРАВИЛЬНОГО ответа (ЗАПРЕЩЕНО):\n"
            f"'Каша 7 злаков стоит 350р, в ней 115 ккал...'\n"
            ...
            f"СПЕЦИАЛЬНО ДЛЯ ЗАВТРАКОВ: Если пользователь спрашивает 'что посоветуешь' или 'что заказать' и в истории недавно (последние 3-5 сообщений) было упоминание завтрака ('завтрак', 'утро', 'с утра'), ОБЯЗАТЕЛЬНО предлагай блюда из категории ЗАВТРАКИ или используй PARSE_CATEGORY:завтраки.\n\n"
            ...
            f"ВАЖНО: Если спрашивают КОНКРЕТНЫЙ НАПИТОК (вино, коктейль, пиво, розовое, игристое, красное, белое) - НЕ ИСПОЛЬЗУЙ SHOW_BAR_PHOTOS! Используй PARSE_CATEGORY:название!\n\n"
            ...
            f"• Если пользователь спрашивает конкретное вино ('розовое', 'игристое', 'красное', 'белое') - используй PARSE_CATEGORY:розовое (или соответствующий тип)\n"
            ...
            f"• 'Какие коктейли есть?' -> 'Смотрите нашу барную карту: PARSE_CATEGORY:коктейли'\n"
            f"• 'Что из горячего?' -> 'Рекомендую наши стейки! PARSE_CATEGORY:горячее'\n\n"
            f"ВАЖНО: Если спрашивают про КОКТЕЙЛИ, БАР или НАПИТКИ (что есть, какие есть, покажи меню) -> ЭТО ЗАПРОС МЕНЮ! ИСПОЛЬЗУЙ PARSE_CATEGORY:коктейли или PARSE_CATEGORY:напитки.\n"
            f"НИКОГДА не используй SHOW_BAR_PHOTOS для вопросов 'какие коктейли есть?' или 'покажи меню бара'. SHOW_BAR_PHOTOS только для вопросов 'как выглядит бар?', 'покажи интерьер бара'.\n\n"
            f"ВАЖНО: Если спрашивают 'ЧТО НОВОГО?', 'НОВИНКИ', 'ЕСТЬ ЧТО-ТО НОВОЕ?' -> ОТВЕЧАЙ:\n"
            f"'Мы постоянно обновляем наше меню, чтобы радовать вас вкусными блюдами! 😋 Вот список новинок:'\n"
            f"И добавь маркер: PARSE_CATEGORY:новинки\n"
            f"Бот сам проверит последние обновления меню и покажет список добавленных блюд.\n\n"
            ...
            f"ФОРМАТ DISH_PHOTO: ТОЛЬКО ПОЛНОЕ название блюда из меню БЕЗ эмодзи и сокращений!\n"
            f"Правильно: DISH_PHOTO:Пицца Инфаркт\n"
            f"Правильно: DISH_PHOTO:Пицца Корона с ветчиной и грибами\n"
            f"Правильно: DISH_PHOTO:Вино ГЕВЮРЦТРАМИНЕР ВАЙНХАУС КАННИС белое п/сухое\n"
            f"Неправильно: DISH_PHOTO:пицца_инфаркт 🍕\n"
            f"Неправильно: DISH_PHOTO:Пицца (СЛИШКОМ КОРОТКО! Пиши полное название!)\n\n"
            ...
            f"ВАЖНО: Вопросы про калории/КБЖУ в КАТЕГОРИИ блюд - это тоже вопросы про меню категории!\n"
            f"Примеры:\n"
            f"- 'Сколько калорий в пицце?' -> PARSE_CATEGORY:пицца (покажи список и скажи 'Смотря в какой именно!')\n"
            f"- 'КБЖУ супов' -> PARSE_CATEGORY:супы (покажи список и скажи 'Смотря в каком именно!')\n"
            ...
            "Если спрашивают про КОНКРЕТНОЕ блюдо ('Сколько калорий в борще?', 'КБЖУ крылышки', 'как выглядит', 'покажи фото', 'что в составе') ИЛИ пишут название блюда (даже полное) - ОБЯЗАТЕЛЬНО используй формат: DISH_PHOTO:название_блюда\n"
            "НЕ используй DISH_PHOTO при перечислении списка блюд!\n\n"
            f"ПРАВИЛО 'ДАВАЙ / УДИВИ МЕНЯ' (ACTIVE RECOMMENDATION):\n"
            f"Если пользователь соглашается на твое предложение рассказать о меню (пишет 'Давай', 'Хорошо', 'Валяй', 'Рассказывай') ИЛИ просит что-то порекомендовать без конкретики ('Посоветуй что-нибудь', 'Удиви меня', 'Что вкусного?'):\n"
            f"1. ЗАПРЕЩЕНО отвечать общими фразами ('У нас есть пицца, супы...'). ЭТО ПЛОХОЙ ОТВЕТ.\n"
            f"2. ТЫ ОБЯЗАН выбрать 1-2 конкретных блюда из MENU_CONTEXT_JSON (или придумать популярные, если контекст пуст: 'Борщ', 'Пицца Пепперони', 'Паста Карбонара') и вкусно их предложить.\n"
            f"3. Пример хорошего ответа: 'Отлично! Рекомендую начать с нашей фирменной Пиццы Пепперони — она сочная и пикантная! Или, если хотите чего-то горячего, попробуйте наш Борщ. Что скажете? Показать фото?'\n"
            f"4. Будь конкретным и инициативным! Предлагай блюда сразу!\n\n"
        )
            ...
            try:
                ...
                if changes:
                    ...
                    menu_updates_context = (
                        ...
                        f"ИНСТРУКЦИЯ: Используй маркер PARSE_CATEGORY:новинки для показа списка новых блюд.\n"
                        f"НЕ пиши никакого текста, система сама покажет список."
                    )
                else:
                    menu_updates_context = (
                        f"КОНТЕКСТ ИЗМЕНЕНИЙ МЕНЮ:\n"
                        f"Пользователь спрашивает о новинках. За последние 30 дней значимых изменений (более {threshold}%) НЕ зафиксировано.\n"
                        f"ИНСТРУКЦИЯ: Используй маркер PARSE_CATEGORY:новинки для показа информации об отсутствии новинок.\n"
                        f"НЕ пиши никакого текста, система сама ответит."
                    )
            except Exception as e:
        ...
        category_translations = {
            ...
            # Вина и подкатегории (для точного поиска)
            'rose': 'розовое', 'rosé': 'розовое', 'розовое': 'розовое',
            'sparkling': 'игристое', 'игристое': 'игристое',
            'red wine': 'красное', 'red': 'красное', 'красное': 'красное',
            'white wine': 'белое', 'white': 'белое', 'белое': 'белое',
        ...
        if 'DISH_PHOTO:' in ai_text:
            ...
            if match:
                ...
                # 🛑 FIX: Защита от усеченных названий (AI может выдать "Пицца" вместо "Пицца Корона...")
                # Если сообщение пользователя длиннее и содержит extracted dish_name, используем сообщение пользователя
                search_query = dish_name
                # Очищаем сообщение от лишних пробелов и эмодзи для проверки длины
                user_msg_clean = message.strip() 
                dish_name_clean = dish_name.strip()
                
                # Список игнорируемых префиксов (чтобы не считать их частью названия)
                ignore_prefixes = ['хочу', 'покажи', 'есть', 'а есть', 'дай', 'можно', 'буду', 'мне']
                user_msg_lower = user_msg_clean.lower()
                
                # Если dish_name очень короткий (например "Пицца"), а пользователь написал длиннее
                # И при этом dish_name содержится в сообщении пользователя
                if len(dish_name_clean) < 10 and len(user_msg_clean) > len(dish_name_clean) + 4:
                     if dish_name_clean.lower() in user_msg_lower:
                         # Пробуем очистить сообщение от префиксов
                         temp_query = user_msg_clean
                         for prefix in ignore_prefixes:
                             if user_msg_lower.startswith(prefix + ' '):
                                 temp_query = temp_query[len(prefix)+1:].strip()
                                 break
                         
                         logger.warning(f"⚠️ AI truncated dish name '{dish_name}'. Using user message '{temp_query}' for search.")
                         search_query = temp_query

                logger.info(f"Итоговый запрос для поиска фото: '{search_query}'")
                ...
                for menu_id, menu in menu_data.items():
                    for category_id, category in menu.get('categories', {}).items():
                        for item in category.get('items', []):
                            ...
                            search_name = search_query.lower().strip()
                            ...
                            if item_name == search_name:
                                ...
                                # Штрафуем, если поиск слишком короткий по сравнению с названием
                                ratio = len(search_name) / len(item_name)
                                score = 60 + (30 * ratio) # 60..90
                            elif search_name in item_name:
                                score = len(search_name) / len(item_name) * 60  # Процент вхождения
                            elif item_name in search_name:
                                # Если название блюда целиком содержится в запросе
                                score = len(item_name) / len(search_name) * 95
        ...
        # 🟢 FIX: Отключаем показ фото бара, если пользователь ищет конкретное вино
        # AI иногда ошибочно добавляет SHOW_BAR_PHOTOS к запросам "розовое", "вино"
        wine_keywords = ['розовое', 'вино', 'игристое', 'красное', 'белое', 'шампанское', 'брют', 'рислинг']
        if show_bar_photos and any(k in message.lower() for k in wine_keywords):
            show_bar_photos = False
            logger.info("SHOW_BAR_PHOTOS disabled due to wine keywords in user message")
