...
async def get_ai_response(message: str, user_id: int) -> Dict:
    ...
    try:
        ...
        # Проверка запроса меню (общая)
        menu_queries = [
            'покажи меню', 'меню', 'хочу меню', 'список меню', 'какое меню', 
            'банкетное меню', 'меню банкетов', 'меню для банкета', 'меню на банкет',
            'посмотреть меню', 'глянуть меню', 'меню ресторана', 'основное меню'
        ]
        
        if any(q in message_lower for q in menu_queries):
             return {
                'type': 'text',
        ...
        system_prompt = (
            ...
            f"КРИТИЧЕСКИ ВАЖНО: Если пользователь задает вопросы, НЕ СВЯЗАННЫЕ с рестораном, едой, мероприятиями или бронированием (например: 'как писать код', 'реши задачу', 'кто такой президент', 'курс доллара', 'представь что ты разработчик' и т.д.) - ТЫ ДОЛЖЕН ОТКАЗАТЬСЯ ОТВЕЧАТЬ.\n"
            ...
            f"КРИТИЧЕСКИ ВАЖНО: Если спрашивают про ЧАСТНЫЕ МЕРОПРИЯТИЯ ('день рождения', 'отметить день рождения', 'свадьба', 'корпоратив', 'юбилей', 'празднование', 'банкет', 'организовать мероприятие', 'провести праздник', 'можно отметить') - АНАЛИЗИРУЙ КОЛИЧЕСТВО ГОСТЕЙ!\n"
            f"• ДО 10 ЧЕЛОВЕК (включительно): Покажи SHOW_BOOKING_OPTIONS\n"
            f"• ОТ 11 ЧЕЛОВЕК И БОЛЕЕ: Покажи SHOW_PRIVATE_EVENT_OPTIONS\n"
            f"Пример: 'Хочу отпраздновать день рождения на 21 человек' → SHOW_PRIVATE_EVENT_OPTIONS\n"
            f"Пример: 'День рождения на 6 человек' → SHOW_BOOKING_OPTIONS\n\n"
            ...
            f"ЕСЛИ МЕНЮ ПУСТОЕ ИЛИ ТЫ НЕ НАШЕЛ БЛЮДО В СПИСКЕ ВЫШЕ - СКАЖИ ЧТО ИНФОРМАЦИЯ УТОЧНЯЕТСЯ, НЕ ПРИДУМЫВАЙ!\n"
            f"ОБЯЗАТЕЛЬНО используй ТОЧНЫЙ маркер PARSE_CATEGORY:ID_КАТЕГОРИИ (ТОЛЬКО ID!) для всех вопросов о категориях:\n"
            f"• 'А у вас есть пиццы?' -> ТОЛЬКО: PARSE_CATEGORY:4853\n"
            f"• 'Только одна пицца?' -> ТОЛЬКО: PARSE_CATEGORY:4853\n"
            f"• 'У вас есть супы?' -> ТОЛЬКО: PARSE_CATEGORY:4819\n"
            f"• 'Какие горячие блюда?' -> ТОЛЬКО: PARSE_CATEGORY:4822\n"
            f"• 'Какие десерты?' -> ТОЛЬКО: PARSE_CATEGORY:4823\n"
            f"• 'Есть ли коктейли?' -> ТОЛЬКО: PARSE_CATEGORY:5583\n"
            f"• 'У вас есть пиво?' -> ТОЛЬКО: PARSE_CATEGORY:4821\n"
            f"• 'Какие напитки?' -> ТОЛЬКО: PARSE_CATEGORY:5583\n"
            f"• 'Сколько калорий в пицце?' -> ТОЛЬКО: PARSE_CATEGORY:4853\n"
            f"• 'Какая калорийность у супов?' -> ТОЛЬКО: PARSE_CATEGORY:4819\n"
            f"• 'Сколько калорий в десертах?' -> ТОЛЬКО: PARSE_CATEGORY:4823\n"
            f"• 'Салаты какие есть?' -> ТОЛЬКО: PARSE_CATEGORY:4818\n"
            f"• 'Покажи салаты' -> ТОЛЬКО: PARSE_CATEGORY:4818\n\n"
            f"ВАЖНЫЕ ID КАТЕГОРИЙ:\n"
            f"- САЛАТЫ: 4818\n"
            f"- ПИЦЦА: 4853\n"
            f"- СУПЫ: 4819\n"
            f"- ГОРЯЧЕЕ: 4822\n"
            f"- ДЕСЕРТЫ: 4823\n"
            f"- НАПИТКИ/ЛИМОНАДЫ: 5583\n"
            f"- СОКИ/ВОДА: 5584\n"
            f"- ПИВО/ЗАКУСКИ К ПИВУ: 4821\n"
            f"- ЗАКУСКИ (СТАРТЕРЫ): 4817\n"
            f"- ПАСТА: 4825\n"
            f"- ГАРНИРЫ: 4820\n\n"
            f"КРИТИЧЕСКИ ВАЖНО: НИКОГДА НЕ ПЕРЕВОДИ маркер PARSE_CATEGORY на русский язык! Используй ID!\n"
            f"ПРАВИЛЬНО: PARSE_CATEGORY:4818\n"
            f"НЕПРАВИЛЬНО: PARSE_CATEGORY:салаты\n"
            f"НЕПРАВИЛЬНО: Парсе категорию: 4818\n"
            f"НЕПРАВИЛЬНО: Парсинг категории: пицца\n"
            f"НЕПРАВИЛЬНО: Категория: пицца\n"
            f"ИСПОЛЬЗУЙ ТОЛЬКО АНГЛИЙСКИЕ БУКВЫ: P-A-R-S-E-_-C-A-T-E-G-O-R-Y\n"
            f"НЕ ОТВЕЧАЙ текстом на вопросы о категориях - ТОЛЬКО маркер PARSE_CATEGORY: НА АНГЛИЙСКОМ!\n"
            ...
            f"КРИТИЧЕСКИ ВАЖНО: Если пользователь отвечает 'хочу', 'да', 'покажи', 'давай' после того как ты предложил посмотреть МЕНЮ КАТЕГОРИИ (пиццы, супы, десерты и т.д.) - ОБЯЗАТЕЛЬНО используй PARSE_CATEGORY:ID_КАТЕГОРИИ!\n"
            f"Примеры:\n"
            f"• Ты: 'Хотите посмотреть наши пиццы?' → Пользователь: 'Хочу' → Ты: PARSE_CATEGORY:4853\n"
            f"• Ты: 'Показать супы?' → Пользователь: 'Да' → Ты: PARSE_CATEGORY:4819\n"
            f"• Ты: 'Посмотрите десерты!' → Пользователь: 'Покажи' → Ты: PARSE_CATEGORY:4823\n"
            f"НЕ отвечай текстом - сразу используй маркер PARSE_CATEGORY! ТОЛЬКО АНГЛИЙСКИМИ БУКВАМИ И ЦИФРАМИ!\n\n"
            ...
            f"• Если недавно говорили о пицце/пиццах - используй PARSE_CATEGORY:4853\n"
            f"• Если недавно говорили о супах - используй PARSE_CATEGORY:4819\n"
            f"• Если недавно говорили о десертах - используй PARSE_CATEGORY:4823\n"
            f"• Если недавно говорили о напитках - используй PARSE_CATEGORY:5583\n"
            f"• Если недавно говорили о пиве - используй PARSE_CATEGORY:4821\n"
            f"• Если недавно говорили о вине - используй PARSE_CATEGORY:5583\n"
            ...
            f"На такие вопросы ОБЯЗАТЕЛЬНО используй ТОЧНЫЙ маркер PARSE_CATEGORY:ID_КАТЕГОРИИ (ТОЛЬКО АНГЛИЙСКИМИ БУКВАМИ И ЦИФРАМИ!)\n"
            f"Примеры:\n"
            f"- 'У вас есть пиво?' -> PARSE_CATEGORY:4821\n"
            f"- 'Есть ли водка?' -> PARSE_CATEGORY:5583\n"
            f"- 'У вас есть салаты?' -> PARSE_CATEGORY:4818\n"
            f"НИКОГДА НЕ ПЕРЕВОДИ PARSE_CATEGORY НА РУССКИЙ! ТОЛЬКО АНГЛИЙСКИЕ БУКВЫ И ЦИФРЫ!\n\n"
            f"ВАЖНО: Вопросы про калории в КАТЕГОРИИ блюд - это тоже вопросы про меню категории!\n"
            f"Примеры:\n"
            f"- 'Сколько калорий в пицце?' -> PARSE_CATEGORY:4853 (покажи список и скажи 'Смотря в какой именно!')\n"
            f"- 'Какая калорийность у супов?' -> PARSE_CATEGORY:4819 (покажи список и скажи 'Смотря в каком именно!')\n"
            f"- 'Сколько калорий в десертах?' -> PARSE_CATEGORY:4823 (покажи список и скажи 'Смотря в каком именно!')\n"
        ...
        # Специальная логика для вопросов про калории в категориях
        calories_category_question = False
        category_name_for_calories = None
        
        message_lower = message.lower()
        if any(word in message_lower for word in ['калори', 'ккал']):
            # Это вопрос про калории - определяем конкретное блюдо или категорию
            specific_dishes = ['борщ', 'маргарита', '4 сыра', 'пепперони', 'инфаркт', 'том ям', 'цезарь']
            is_specific_dish = any(dish in message_lower for dish in specific_dishes)
            
            if not is_specific_dish:
                # Это вопрос про калории в категории
                if 'пицц' in message_lower:
                    category_name_for_calories = 'пицца'
                    calories_category_question = True
                    logger.info(f"🔍 Обнаружен вопрос про калории в пицце")
                elif 'суп' in message_lower:
                    category_name_for_calories = 'суп'
                    calories_category_question = True
                    logger.info(f"🔍 Обнаружен вопрос про калории в супах")
                elif 'десерт' in message_lower:
                    category_name_for_calories = 'десерт'
                    calories_category_question = True
                    logger.info(f"🔍 Обнаружен вопрос про калории в десертах")
                elif 'салат' in message_lower:
                    category_name_for_calories = 'салаты'
                    calories_category_question = True
                    logger.info(f"🔍 Обнаружен вопрос про калории в салатах")
...
def get_fallback_response(message: str, user_id: int) -> Dict:
    ...
    # Специальная обработка вопросов про калории в категориях (fallback версия)
    if any(word in message_lower for word in ['калори', 'ккал', 'калорийность']):
        specific_dishes = ['борщ', 'маргарита', '4 сыра', 'пепперони', 'инфаркт', 'том ям', 'цезарь']
        is_specific_dish = any(dish in message_lower for dish in specific_dishes)
        ...
        if not is_specific_dish:
            # Это вопрос про калории в категории - показываем КРАТКИЙ список с вопросом
            if any(word in message_lower for word in ['пицц', 'пиза']):
                logger.info(f"🔍 Fallback: Обнаружен вопрос про калории в пицце - показываем краткий список")
                return {
                    'type': 'text',
                    'text': '🍕 У нас есть отличные пиццы! Смотря в какой именно вас интересуют калории:',
                    'show_category_brief': 'пицца'
                ...
                logger.info(f"🔍 Fallback: Обнаружен вопрос про калории в супах - показываем краткий список")
                return {
                    'type': 'text',
                    'text': '🍲 У нас есть отличные супы! Смотря в каком именно вас интересует калорийность:',
                    'show_category_brief': 'суп'
                ...
                logger.info(f"🔍 Fallback: Обнаружен вопрос про калории в десертах - показываем краткий список")
                return {
                    'type': 'text',
                    'text': '🍰 У нас есть отличные десерты! Смотря в каком именно вас интересует калорийность:',
                    'show_category_brief': 'десерт'
                ...
                logger.info(f"🔍 Fallback: Обнаружен вопрос про калории в салатах - показываем краткий список")
                return {
                    'type': 'text',
                    'text': '🥗 У нас есть отличные салаты! Смотря в каком именно вас интересуют калории:',
                    'show_category_brief': 'салаты'
    ...
    # ПУСТЬ ВСЁ ДЕЛАЕТ ИИ! Удаляем жесткие перехваты "да" и коротких ответов.
    # Если это просто "да" или короткий ответ, мы НЕ будем здесь возвращать заглушку, 
    # а позволим сообщению уйти в основной AI цикл (get_ai_response).
    ...
    # Обработка подтверждения показа банкетного персонажа - ПУСТЬ ДЕЛАЕТ ИИ
    # Мы удалили ручной перехват, чтобы Polza AI сама решала, когда генерировать картинку.
    
    # Все остальные короткие ответы и проверки удалены по требованию "ПУСТЬ ВСЁ ДЕЛАЕТ ИИ!"
    # Теперь все запросы (кроме явных команд меню/старт) будут уходить в Polza AI.
