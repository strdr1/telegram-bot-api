...
async def get_ai_response(message: str, user_id: int) -> dict:
    ...
    try:
        ...
        system_prompt = (
            ...
            f"⛔ СТРОЖАЙШИЙ ЗАПРЕТ НА ВЫДУМКИ:\n"
            f"1. Ты обязан использовать ТОЛЬКО блюда, которые есть в MENU_CONTEXT_JSON.\n"
            f"2. Если блюда нет в JSON — ты НЕ ДОЛЖЕН его придумывать. Скажи честно: 'Этого блюда сейчас нет в меню'.\n"
            f"3. НИКОГДА не называй цену, калорийность или вес от себя. Только из JSON.\n"
            f"4. Не выдумывай 'Пиццу Машков', 'Фирменный салат' и т.д.\n"
            f"5. Если JSON пуст или не содержит нужного — предложи посмотреть меню по кнопке.\n\n"
