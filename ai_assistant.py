...
async def get_ai_response(message: str, user_id: int) -> Dict:
    ...
    try:
        ...
        system_prompt = (
            ...
            f"ЕСЛИ ЧЕЛОВЕК ПИШЕТ 'МОРЕПРОДУКТЫ' ИЛИ СПРАШИВАЕТ ПРО МОРЕПРОДУКТЫ (КРЕВЕТКИ, МИДИИ, КАЛЬМАРЫ, ОСЬМИНОГА И Т.Д.), ВСЕГДА ИСПОЛЬЗУЙ ДЛЯ SEARCH КОРНИ СЛОВ: 'креветк', 'кальмар', 'мид', 'осьминог', 'гребешк', 'краб'.\n"
            ...
            f"ВАЖНО: Если пользователь просит 'А ещё?' или 'А ещё что-то?' после того, как ты уже предложил блюдо, старайся предложить ДРУГОЕ, НОВОЕ блюдо, а не повторять одно и то же. Если подходящих вариантов больше нет, честно скажи об этом.\n\n"
            ...
            f"1. НИКОГДА НЕ ПЕРЕЧИСЛЯЙ БЛЮДА ВРУЧНУЮ СПИСКОМ! НИКОГДА НЕ ПИШИ ЦЕНЫ САМ!\n"
            f"2. ВСЕГДА используй SEARCH:запрос или PARSE_CATEGORY:категория, чтобы система сама показала список.\n"
            f"3. Если спрашивают 'Что посоветуешь?' (без конкретики) - спроси о предпочтениях (мясо, рыба, пицца) или предложи посмотреть популярные категории.\n"
            f"   Пример: 'У нас всё очень вкусное! Вы предпочитаете сытные мясные блюда, легкие салаты или, может быть, пиццу?'\n"
            f"4. Если спрашивают рекомендацию С КОНКРЕТИКОЙ ('Посоветуй что-то мясное') - используй SEARCH:мяс\n"
            f"Пример:\n"
