...
async def get_ai_response(message: str, user_id: int) -> Dict:
    ...
    try:
        ...
        system_prompt = (
            ...
            f"🔍 ПОИСК БЛЮД ПО ИНГРЕДИЕНТАМ:\n"
            f"Если пользователь ищет блюда с определенными ингредиентами (например: 'блюда с креветками', 'что есть с грибами', 'острое', 'том ям с морепродуктами'), используй маркер SEARCH:\n"
            f"✅ ПРАВИЛЬНО: SEARCH:креветки, том ям\n"
            f"✅ ПРАВИЛЬНО: SEARCH:грибы\n"
            f"ТЫ МОЖЕШЬ УКАЗЫВАТЬ НЕСКОЛЬКО КЛЮЧЕВЫХ СЛОВ ЧЕРЕЗ ЗАПЯТУЮ! Выбирай самые важные ингредиенты из запроса.\n"
