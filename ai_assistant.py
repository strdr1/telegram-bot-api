...
async def get_ai_response(message: str, user_id: int) -> Dict:
    ...
    try:
        ...
        # Специальная обработка контекстных вопросов (до обращения к AI)
        ...
        system_prompt = (
            ...
            f"ОБЯЗАТЕЛЬНО используй ТОЧНЫЙ маркер PARSE_CATEGORY: (ТОЛЬКО НА АНГЛИЙСКОМ!) для всех вопросов о категориях:\n"
            ...
            f"• 'Сколько калорий в пицце?' -> ТОЛЬКО: PARSE_CATEGORY:пицца\n"
            f"• 'Какая калорийность у супов?' -> ТОЛЬКО: PARSE_CATEGORY:суп\n"
            f"• 'Сколько калорий в десертах?' -> ТОЛЬКО: PARSE_CATEGORY:десерт\n\n"
            f"КРИТИЧЕСКИ ВАЖНО: НИКОГДА НЕ ПЕРЕВОДИ маркер PARSE_CATEGORY на русский язык!\n"
            f"ПРАВИЛЬНО: PARSE_CATEGORY:пицца\n"
            f"НЕПРАВИЛЬНО: Парсе категорию: пицца\n"
            f"НЕПРАВИЛЬНО: ПАРС_КАТЕГОРИЯ:пицца\n"
            f"НЕПРАВИЛЬНО: Парсинг категории: пицца\n"
            ...
            f"• Ты: 'Показать супы?' → Пользователь: 'Да' → Ты: PARSE_CATEGORY:суп\n"
            f"• Ты: 'Посмотрите десерты!' → Пользователь: 'Покажи' → Ты: PARSE_CATEGORY:десерт\n"
            ...
            f"• Если недавно говорили о супах - используй PARSE_CATEGORY:суп\n"
            f"• Если недавно говорили о десертах - используй PARSE_CATEGORY:десерт\n"
            ...
            f"На такие вопросы ОБЯЗАТЕЛЬНО используй ТОЧНЫЙ маркер PARSE_CATEGORY:X (ТОЛЬКО АНГЛИЙСКИМИ БУКВАМИ!)\n"
            f"Примеры:\n"
            f"- 'У вас есть пиво?' -> PARSE_CATEGORY:пиво\n"
            f"- 'Есть ли водка?' -> PARSE_CATEGORY:водка\n"
            f"- 'У вас есть салаты?' -> PARSE_CATEGORY:салаты\n"
            f"НИКОГДА НЕ ПЕРЕВОДИ PARSE_CATEGORY НА РУССКИЙ! ТОЛЬКО АНГЛИЙСКИЕ БУКВЫ!\n\n"
            ...
            f"- 'Какая калорийность у супов?' -> PARSE_CATEGORY:суп (покажи список и скажи 'Смотря в каком именно!')\n"
            f"- 'Сколько калорий в десертах?' -> PARSE_CATEGORY:десерт (покажи список и скажи 'Смотря в каком именно!')\n"
...
def get_fallback_response(message: str, user_id: int) -> Dict:
    ...
    # Проверяем короткие ответы и контекстные вопросы
    ...
    # Для контекстных вопросов типа "А какие есть?" - проверяем историю на предмет категорий
    if any(phrase in message_lower for phrase in context_questions):
        # Проверяем историю пользователя на предмет недавних упоминаний категорий
        if user_id in user_history:
            recent_messages = user_history[user_id][-5:]  # Последние 5 сообщений
            for msg in reversed(recent_messages):
                ...
                # Ищем упоминания категорий в недавней истории
                if 'пицц' in content:
                    logger.info(f"🔍 Fallback: Обнаружен контекст пиццы в истории, показываем пиццы")
                    return {
                        'type': 'text',
                        'text': '🍕 У нас есть отличные пиццы!',
                        'show_category_brief': 'пицца'
                    ...
                    logger.info(f"🔍 Fallback: Обнаружен контекст супов в истории, показываем супы")
                    return {
                        'type': 'text',
                        'text': '🍲 У нас есть отличные супы!',
                        'show_category_brief': 'суп'
                    }
                elif 'десерт' in content:
                    logger.info(f"🔍 Fallback: Обнаружен контекст десертов в истории, показываем десерты")
                    return {
                        'type': 'text',
                        'text': '🍰 У нас есть отличные десерты!',
                        'show_category_brief': 'десерт'
                    }
                elif 'салат' in content:
                    logger.info(f"🔍 Fallback: Обнаружен контекст салатов в истории, показываем салаты")
                    return {
                        'type': 'text',
                        'text': '🥗 У нас есть отличные салаты!',
                        'show_category_brief': 'салаты'
                    ...
                    logger.info(f"🔍 Fallback: Обнаружен контекст пива в истории, показываем пиво")
                    return {
                        'type': 'text',
                        'text': '🍺 У нас есть отличное пиво!',
                        'show_category_brief': 'пиво'
                    ...
                    logger.info(f"🔍 Fallback: Обнаружен контекст вина в истории, показываем вино")
                    return {
                        'type': 'text',
                        'text': '🍷 У нас есть отличное вино!',
                        'show_category_brief': 'вино'
        ...
        # Если контекст не найден - общий ответ
        return {
            'type': 'text',
            'text': '🤔 Какие именно? Уточните, пожалуйста - пиццы, супы, салаты, десерты или что-то другое? 🍽️'
        }
    ...
    if 'пицца' in message_lower:
        if '4 сыра' in message_lower or 'четыре сыра' in message_lower:
            ...
            # Для простых запросов о пиццах показываем краткий список
            return {
                'type': 'text',
                'text': '🍕 У нас есть отличные пиццы!',
                'show_category_brief': 'пицца'
