...
async def get_ai_response(message: str, user_id: int) -> Dict:
    ...
    try:
        ...
        system_prompt = (
            ...
            f"Если пользователь ищет блюда не по категории, а по ИНГРЕДИЕНТАМ, ТИПУ или ОСОБЕННОСТЯМ (например: 'с овощами', 'мясное', 'без мяса', 'веганское', 'постное', 'острое', 'с грибами', 'с сыром') - ИСПОЛЬЗУЙ МАРКЕР SEARCH:запрос\n"
            f"ПРИМЕРЫ:\n"
            f"• 'Есть что-то с овощами?' -> 'Конечно! Вот что у нас есть с овощами:' + SEARCH:овощи\n"
            f"• 'Хочу мясное' -> 'Для любителей мяса у нас отличный выбор:' + SEARCH:мясо\n"
            f"• 'Что есть с грибами?' -> 'С грибами есть несколько вкусных блюд:' + SEARCH:грибы\n"
            ...
            f"НЕ ИСПОЛЬЗУЙ PARSE_CATEGORY для таких запросов (веган, сыр, грибы и т.д.), так как таких категорий НЕТ! Используй ТОЛЬКО SEARCH! В SEARCH пиши только КОРЕНЬ слова (веган, гриб, сыр) для лучшего поиска!\n"
            ...
            f"Если пользователь спрашивает о блюдах (например, 'есть ли вегетарианское?', 'что с мясом?'):\n"
            f"1. СНАЧАЛА ответь на вопрос, перечислив подходящие блюда из контекста (названия, цены).\n"
            f"2. ЗАТЕМ, если хочешь показать кнопку категории, добавь маркер PARSE_CATEGORY:название\n"
            f"Пример:\n"
            f"User: 'Есть что-то вегетарианское?'\n"
            f"AI: 'Да, у нас есть Греческий салат (450₽) и Овощное рагу. Также есть вегетарианская пицца. PARSE_CATEGORY:вегетарианское'\n\n"
            ...
            f"1. СНАЧАЛА ПОИЩИ В 'ПОЛНОЕ МЕНЮ РЕСТОРАНА' (см. ниже) блюда, соответствующие запросу (по названию или составу).\n"
            f"   - Если нашел конкретные блюда (например, 'Стейк Рибай' в категории 'Горячее') - ОТВЕТЬ ТЕКСТОМ, перечислив их!\n"
            f"   - Пример: 'Да, у нас есть стейки: Стейк Рибай (1200р) и Стейк Мачете (900р). Хотите посмотреть раздел Горячее?'\n"
            f"   - В конце ответа добавь PARSE_CATEGORY:Название_Категории (если блюда из одной категории).\n"
            f"2. Если запрос совпадает с названием КАТЕГОРИИ (пицца, супы, роллы) - используй PARSE_CATEGORY:Категория.\n"
            f"3. НИКОГДА не отвечай 'Я не нашел категорию', если ты видишь подходящие блюда в списке!\n"
            f"Примеры:\n"
            f"- 'Есть стейки?' -> 'Да, конечно! У нас есть Стейк Рибай и Стейк из лосося. Посмотрите наше горячее меню!' + PARSE_CATEGORY:горячее\n"
            f"- 'Что есть мясное?' -> 'Любителям мяса рекомендую: Бефстроганов, Котлеты из щуки, Стейки. Взгляните на горячие блюда.' + PARSE_CATEGORY:горячее\n"
        ...
        if 'parse_category:' in ai_text.lower():
            ...
            if match:
                ...
                history_text = f"🔍 Ищу блюда по запросу '{search_query}'"
