...
async def get_ai_response(message: str, user_id: int) -> Dict:
    ...
    try:
        ...
        system_prompt = (
            ...
            f"ОБЯЗАТЕЛЬНО используй ТОЧНЫЙ маркер PARSE_CATEGORY: (НА АНГЛИЙСКОМ!) для всех вопросов о категориях:\n"
            ...
            f"КРИТИЧЕСКИ ВАЖНО: НЕ ПЕРЕВОДИ маркер PARSE_CATEGORY на русский язык!\n"
            ...
            f"НЕ отвечай текстом - сразу используй маркер PARSE_CATEGORY!\n\n"
            ...
            f"На такие вопросы ОБЯЗАТЕЛЬНО используй ТОЧНЫЙ маркер PARSE_CATEGORY:X (НА АНГЛИЙСКОМ!)\n"
            f"Примеры:\n"
