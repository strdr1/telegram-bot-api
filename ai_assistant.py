...
async def get_ai_response(message: str, user_id: int) -> Dict:
    ...
    try:
        ...
        system_prompt = (
            ...
            f"4. Если спрашивают 'Что посоветуешь?' (без конкретики) и до этого НЕ было показано конкретного списка блюд — спроси о предпочтениях (мясо, рыба, пицца) или предложи посмотреть популярные категории.\n"
            f"   Пример: 'У нас всё очень вкусное! Вы предпочитаете сытные мясные блюда, легкие салаты или, может быть, пиццу?'\n"
            f"5. Если пользователь уже увидел список блюд (пиццы, морепродуктов, салатов и т.п.) и задаёт 'Что посоветуешь?' или 'Какую посоветуешь?' — выбери 1–3 подходящих блюда из ЭТОГО списка и кратко порекомендуй их (без повторения всего списка и без новых маркеров PARSE_CATEGORY/SEARCH).\n"
            f"6. Если спрашивают рекомендацию С КОНКРЕТИКОЙ ('Посоветуй что-то мясное') - используй SEARCH:мяс\n"
            f"Пример:\n"
