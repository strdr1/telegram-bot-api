...
async def get_ai_response(message: str, user_id: int) -> Dict:
    ...
    try:
        ...
        system_prompt = (
            ...
            f"• 'А у вас есть пиццы?' -> ТОЛЬКО: PARSE_CATEGORY:3087\n"
            f"• 'Только одна пицца?' -> ТОЛЬКО: PARSE_CATEGORY:3087\n"
            f"• 'У вас есть супы?' -> ТОЛЬКО: PARSE_CATEGORY:4746\n"
            f"• 'Какие горячие блюда?' -> ТОЛЬКО: PARSE_CATEGORY:4730\n"
            f"• 'Какие десерты?' -> ТОЛЬКО: PARSE_CATEGORY:4748\n"
            f"• 'Есть ли коктейли?' -> ТОЛЬКО: PARSE_CATEGORY:4744\n"
            f"• 'У вас есть пиво?' -> ТОЛЬКО: PARSE_CATEGORY:4744\n"
            f"• 'Какие напитки?' -> ТОЛЬКО: PARSE_CATEGORY:4744\n"
            f"• 'Сколько калорий в пицце?' -> ТОЛЬКО: PARSE_CATEGORY:3087\n"
            f"• 'Какая калорийность у супов?' -> ТОЛЬКО: PARSE_CATEGORY:4746\n"
            f"• 'Сколько калорий в десертах?' -> ТОЛЬКО: PARSE_CATEGORY:4748\n"
            ...
            f"Примеры:\n"
            f"• Ты: 'Хотите посмотреть наши пиццы?' → Пользователь: 'Хочу' → Ты: PARSE_CATEGORY:3087\n"
            f"• Ты: 'Показать супы?' → Пользователь: 'Да' → Ты: PARSE_CATEGORY:4746\n"
            f"• Ты: 'Посмотрите десерты!' → Пользователь: 'Покажи' → Ты: PARSE_CATEGORY:4748\n"
            ...
            f"• Если недавно говорили о пицце/пиццах - используй PARSE_CATEGORY:3087\n"
            f"• Если недавно говорили о супах - используй PARSE_CATEGORY:4746\n"
            f"• Если недавно говорили о десертах - используй PARSE_CATEGORY:4748\n"
            f"• Если недавно говорили о напитках - используй PARSE_CATEGORY:4744\n"
            f"• Если недавно говорили о пиве - используй PARSE_CATEGORY:4744\n"
            f"• Если недавно говорили о вине - используй PARSE_CATEGORY:4744\n"
            ...
            f"Примеры:\n"
            f"- 'У вас есть пиво?' -> PARSE_CATEGORY:4744\n"
            f"- 'Есть ли водка?' -> PARSE_CATEGORY:4744\n"
            ...
            f"Примеры:\n"
            f"- 'Сколько калорий в пицце?' -> PARSE_CATEGORY:3087 (покажи список и скажи 'Смотря в какой именно!')\n"
            f"- 'Какая калорийность у супов?' -> PARSE_CATEGORY:4746 (покажи список и скажи 'Смотря в каком именно!')\n"
            f"- 'Сколько калорий в десертах?' -> PARSE_CATEGORY:4748 (покажи список и скажи 'Смотря в каком именно!')\n"
