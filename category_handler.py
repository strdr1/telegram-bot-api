...
def find_dishes_by_name(raw_search: str, limit: int = 20, include_alcohol: bool = False) -> list:
    ...
    alcohol_roots = ['вино', 'винн', 'пиво', 'пивн', 'алкоголь', 'коктейль', 'водка', 'виски', 'ром', 'текила', 'разливное', 'бутылочное', 'шампанское', 'игристое', 'брют', 'сидр', 'настойк']
...
async def handle_show_category_brief(category_name: str, user_id: int, bot, intro_message: str = None, force_list: bool = False):
    ...
    """
    try:
        # � ПЕРЕХВАТ ПОИСКА НОВИНОК (до всех проверок)
        # Если ищут новинки - обрабатываем сразу
        search_name_lower = str(category_name).lower().strip()
        is_new_search_global = any(root in search_name_lower for root in ['новинк', 'новое', 'new'])
        
        if is_new_search_global:
             # Если ищут новинки - показываем реально новые товары (diff snapshot)
             new_items = get_new_items_list()
             if new_items:
                 category_title = "🆕 Новинки меню"
                 text = f"🆕 <b>{category_title}</b>\n\nВот что у нас появилось новенького!\n\n"
                 ...
                 return text
                  ...
                  # Если новинок нет
                  text = "В последнем обновлении меню новых позиций не добавилось. Всё стабильно вкусно! 😋"
                  await safe_send_message(bot, user_id, text, parse_mode="HTML")
                  return "Список новинок пуст"
...
async def handle_show_category(category_name: str, user_id: int, bot, intro_message: str = None, is_search: bool = False):
    ...
    try:
        ...
        # 🆕 ПЕРЕХВАТ ПОИСКА НОВИНОК (до цикла по меню)
        # Если ищут новинки - обрабатываем сразу, не заходя в цикл категорий
        search_name_lower = str(category_name).lower().strip()
        is_new_search_global = any(root in search_name_lower for root in ['новинк', 'новое', 'new'])
        ...
        if is_new_search_global:
             ...
             if new_items:
                 ...
                 text += f"\n💡 <i>Спросите про конкретное блюдо, чтобы увидеть фото!</i>"
                 await safe_send_message(bot, user_id, text, parse_mode="HTML")
                 # 🛑 ВАЖНО: Возвращаем строку (summary), чтобы вызывающий код (handlers_main) знал, что обработка успешна
                 return f"Показан список новинок ({len(new_items)} блюд)"
             else:
                  # Если новинок нет
                  text = "В последнем обновлении меню новых позиций не добавилось. Всё стабильно вкусно! 😋"
                  await safe_send_message(bot, user_id, text, parse_mode="HTML")
                  # 🛑 ВАЖНО: Возвращаем строку (summary)
                  return "Список новинок пуст"
